...
class Ion_auth_model extends CI_Model
{
	...
	/*
	 * Admin OR Library Admin check
	 *
	 */
	
	public function is_admin_validate($userId){
	...
	public function getLibraryId($userId){
		$query = $this->db->select('library_id')
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		//$query = $this->db->select('users.'.$this->identity_column . ', users.email, id, password, active, last_login')
		//					->join('school_library_relation', 'users.school_id = school_library_relation.school_id')
		//					->join('libraries', 'libraries.library_id = school_library_relation.library_id')
		//					->where('users.'.$this->identity_column, $identity)
		//					->where("libraries.library_slug = '".$_SESSION['in_sub_domain']['lib_id']."'")
		//					->limit(1)
		//					->order_by('id', 'desc')
		//					->get($this->tables['users']);
		...
		$query = $this->db->select('users.'.$this->identity_column . ', users.email, id, password, active, last_login')
							->where('users.'.$this->identity_column, $identity)
							->where("users.library_id = '".$_SESSION['in_sub_domain']['original_library_id']."'")
							->limit(1)
							->order_by('id', 'desc')
							->get($this->tables['users']);
